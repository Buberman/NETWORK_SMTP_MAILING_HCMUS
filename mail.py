...
def smtp_client(HOST,SMTP_PORT,CLIENT_DOMAIN,SENDER):
        ...
        for i in range(num_attachments):
            while True:
                ...
                if os.path.isfile(attachment_path):
                    file_size = os.path.getsize(attachment_path) / (1024 * 1024)  # Convert to MB
                    if file_size <= MAX_ATTACHMENT_SIZE_MB:
                        attachments.append(attachment_path)
                        break  # Exit the loop if the file is valid
                    else:
                        print(f"File '{attachment_path}' lớn hơn {MAX_ATTACHMENT_SIZE_MB} MB. Vui lòng chọn file khác.")
                else:
                    print("Đường dẫn không tồn tại hoặc không thể truy cập. Vui lòng thử lại.")
    ...
    for attachment_path in attachments:
        if os.path.isfile(attachment_path):
            ...
            if file_size > MAX_ATTACHMENT_SIZE_MB * 1024 * 1024:  # Size in bytes (3 MB)
...
def list_emails_in_folder(mailbox_path, folder_name):
    ...
    for idx, mail in enumerate(all_mails, 1):
        ...
        read_status = '' if is_email_read('C:\\data', folder_name, mail) else '(Chưa đọc)'
        sender, subject, to_addresses, cc_addresses, bcc_addresses, content, attachments = parse_eml_file(mail_path)
        print(f"{idx}.{read_status} {sender} - {subject}")
    return all_mails
        ...
        if mail_choice == '':
            ...
            try:
                ...
                if 1 <= mail_index <= len(all_mails):
                    ...
                    mark_email_as_read('C:\\data', selected_folder, mail_filename)
...
def load_email_status(mailbox_path):
    status_file = os.path.join(mailbox_path, 'email_status.json')
    if os.path.exists(status_file):
        with open(status_file, 'r') as file:
            return json.load(file)
    return {}

def save_email_status(mailbox_path, status):
    status_file = os.path.join(mailbox_path, 'email_status.json')
    with open(status_file, 'w') as file:
        json.dump(status, file)

def mark_email_as_read(mailbox_path, folder_name, mail_id):
    status = load_email_status(mailbox_path)
    if folder_name not in status:
        status[folder_name] = {}
    status[folder_name][mail_id] = True
    save_email_status(mailbox_path, status)

def is_email_read(mailbox_path, folder_name, mail_id):
    status = load_email_status(mailbox_path)
    return status.get(folder_name, {}).get(mail_id, False)
